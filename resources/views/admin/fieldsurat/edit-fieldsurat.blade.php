    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="p-6 bg-white shadow-sm sm:rounded-lg">
                ...
                <form method="POST" action="{{ route('fieldsurat.update', $fieldsurat->id) }}">
                    @csrf
                    @method('PUT')
                        ...
                        <select id="jenis_surat_id" name="jenis_surat_id"
                            ...
                            @foreach ($jenisSurats as $jenis)
                                <option value="{{ $jenis->id }}"
                                    {{ $fieldsurat->jenis_surat_id == $jenis->id ? 'selected' : '' }}>
                                    {{ $jenis->nama_jenis }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <x-text-input id="nama_field" name="nama_field" type="text" class="block w-full mt-1"
                            value="{{ old('nama_field', $fieldsurat->nama_field) }}" required />
                    ...
                    <div class="mb-4">
                        ...
                        <select id="tipe_field" name="tipe_field" class="block w-full mt-1 border-gray-300 rounded-md">
                            @foreach (['text', 'number', 'date', 'time', 'boolean', 'email', 'file', 'select'] as $tipe)
                                <option value="{{ $tipe }}"
                                    {{ $fieldsurat->tipe_field == $tipe ? 'selected' : '' }}>
                                    {{ ucfirst($tipe) }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="options" name="options" rows="3" class="block w-full mt-1 border-gray-300 rounded-md">{{ old('options', is_array($fieldsurat->options) ? implode(',', $fieldsurat->options) : '') }}</textarea>
                        <x-input-error :messages="$errors->get('options')" class="mt-2" />
                    </div>

                    <div class="flex items-center mb-4">
                        <input id="is_required" name="is_required" type="checkbox" value="1"
                            class="border-gray-300 rounded" {{ $fieldsurat->is_required ? 'checked' : '' }}>
